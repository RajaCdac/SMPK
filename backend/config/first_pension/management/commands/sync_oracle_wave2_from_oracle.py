...
from first_pension.services.oracle_wave2_sync import (
    WAVE2_SYNC_SPECS,
    sync_all_wave2,
    sync_wave2_table,
)
...
class Command(BaseCommand):
    help = (
        "Import Wave 2 Oracle tables into MySQL: pension proposals, earn/dedn lines, "
        "applications, oldbill params, salout, and employee reference (ADM/PER/FIN/FINSCALE)."
    )
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--table",
            dest="table",
            help="Sync one Oracle table only (e.g. FI_PN_MH_PENSION_PROPOSAL)",
        )
        parser.add_argument(
            "--emp",
            dest="emp_cd",
    ...
    def handle(self, *args, **options):
        ...
        if table_filter:
            spec = next(
                (s for s in WAVE2_SYNC_SPECS if s["oracle_table"] == table_filter),
                None,
            )
            if not spec:
                known = ", ".join(s["oracle_table"] for s in WAVE2_SYNC_SPECS)
                self.stderr.write(
